@extends('layouts.app-guest', ['class' => 'bg-info', 'activePage' => '', 'title' => __('Material Dashboard')])

@section('content')
    <div class="container mt-5" style="height: auto;">
        <div class="row align-items-center">
            <div class="col-lg-4 col-md-6 col-sm-8 ml-auto mr-auto mt-5">
                <form class="form mt-5" method="POST" action="{{ route('password.update') }}">
                    @csrf

                    <input type="hidden" name="token" value="{{ $token }}">
                    <div class="card card-login card-hidden mb-3">
                        <div class="card-header card-header-primary text-center">
                            <h4 class="card-title"><strong>{{ __('Reinicio de Password') }}</strong></h4>
                        </div>
                        <div class="card-body ">
                        ...
                        <div class="card-footer justify-content-center mx-auto">
                            <button type="submit" class="btn btn-primary">{{ __('Reiniciar Password') }}</button>
                        </div>
                    </div>
                </form>
            </div>
