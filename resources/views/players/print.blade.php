...
<body>
    @foreach ($predictions as $player)
        <h3>Jugador {{ $player->players->name }} </h3>
        <table class="table table-striped table-inverse table-responsive" style="width:90%">
            ...
            <tbody>
                @foreach ($player->predictionDetails as $prediction)
                    <tr style="font-size: 12.5px">
                        ...
                        <td style="text-align: start; margin-left:-30px">
                            ...
                            <img src="{{ asset('images') }}/{{ $prediction->matchup->teamB->url_flag }}"
        ...
        <span style="font-size:10px"> {{ $player->uuid }} </span>
